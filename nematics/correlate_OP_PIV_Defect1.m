...
%
%
dirOP = dir(['F:\GD\DATA\HT1080\Orient'  '\*.tif']);
dirPIV = dir(['F:\GD\DATA\HT1080\PIV_DATs'  '\*.mat']);
...
for i = 1:size(dirOP,1)
    ...
    %     p3=strfind(nameOP,'O');
    p4=strfind(nameOP,'.');
    p5=strfind(nameOP,'old_');
    p6=strfind(nameOP,'old');
    p7=strfind(nameOP,'09_07_2018');
    
    if p2(1)<p1(1)
        patternOP = nameOP(p1:p4-1);
    else
        patternOP = nameOP(p1:p2(end)-1);
    end
    
    if ~isempty(p5)
        patternOP = ['old_' patternOP];
    elseif ~isempty(p6) && isempty(p5)
        patternOP = ['old' patternOP];
    end
    if ~isempty(p7)
        patternOP = [patternOP '.tif_09'];
    end
    
    for j = 1:size(dirPIV,1)
        namePIV = dirPIV(j).name;
        if contains(namePIV,patternOP)
            if isempty(p7)
...
load('F:\GD\Curie\DESKTOP\HT1080\shear_OP_correlation_new.mat')
for i=1:size(dirOP,1)
