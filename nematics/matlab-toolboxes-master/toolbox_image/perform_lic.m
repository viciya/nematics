...
if flow_correction
    ...
    dX(dX>ww/2) = dX(dX>ww/2)-(ww-1); dX(dX<-ww/2) = dX(dX<-ww/2)+(ww-1);
    dY(dY>ll/2) = dY(dY>ll/2)-(ll-1); dY(dY<-ll/2) = dY(dY<-ll/2)+(ll-1);
    d = sqrt(dX.^2 + dY.^2);
    d = repmat( mean(mean(d)), [ll ww 1] ) - d;
...
% compute averaging
M = zeros(ll,ww);
[Y,X] = meshgrid(1:ww,1:ll);
W = M;
for i=1:p
    A = interp2(1:ww,1:ll, M0, H(:,:,2,i), H(:,:,1,i) );
    if flow_correction
        w = d(:,:,i)<eta;
    else
        w = ones(ll,ww);
