...
figure(21);
vstep = 1;
vvDown = (vv>0).*vv;uuDown = (vv>0).*uu;
q0 = quiver(X(1:vstep:end,1:vstep:end),Y(1:vstep:end,1:vstep:end),...
    uuDown(1:vstep:end,1:vstep:end),vvDown(1:vstep:end,1:vstep:end),1.5);
q0.LineWidth=1.5;
q0.Color = [.1 .2 .8];hold on
vvUp = (vv<0).*vv;uuUp = (vv<0).*uu;
q1 = quiver(X(1:vstep:end,1:vstep:end),Y(1:vstep:end,1:vstep:end),...
    uuUp(1:vstep:end,1:vstep:end),vvUp(1:vstep:end,1:vstep:end),1.5);
q1.LineWidth=1.5;
q1.Color = [.8 .2 .1];
hold off
