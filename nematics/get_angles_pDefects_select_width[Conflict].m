...
% histogram(Q(:,1),30); 
figure(1346);
% histogram(Q(Q(:,1)<100,3),60,'Normalization', 'PDF');  hold on
histogram(-Q(:,3),60,'Normalization', 'PDF');  hold on

figure(1345);
polarhistogram(-Q(Q(:,1)<40,3)/180*pi,'Normalization', 'PDF');  hold on
...
%% PLOT DEFECT LEFT, RIGHT edge
%  for the RIGHT edge -Q(:,3) intead of Q(:,3)
clearvars PdefPOS_S PdefPOS_1 Q QQ
PdefPOS_S = PdefPOS_R;
emptyTF = cellfun(@isempty,PdefPOS_S); %check for empty cells
fileY = ~emptyTF; %sum(~emptyTF)
PdefPOS_1 = PdefPOS_S(fileY);
...
% h2 = histogram(centerQ,'Normalization', 'PDF'); hold on
% h2.EdgeAlpha = 0; h2.FaceColor = [1 0 0]; 
[Qn,binQ] = histcounts(-Q(:,3),60,'Normalization', 'PDF');
plot(mean([binQ(2:end);binQ(1:end-1)])',Qn','r','LineWidth',2); hold on


axis([-180 180 0 1e-2]); xticks([-180 -90 0 90 180]);
set(gca,'Fontsize',18); 
% title(length(sRange))
ylabel('$ PDF $','Interpreter','latex','FontSize',28);
xlabel('$Orientation\ of\ + 1/2\ defect\ (deg)$','Interpreter','latex','FontSize',24);
legend('edge-free','left edge','right edge')
%
figure(310);
polarhistogram(-Q(:,3)/180*pi,30,'Normalization', 'PDF');  hold on
