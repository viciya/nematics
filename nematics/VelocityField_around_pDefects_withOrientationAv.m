...
%     try
    ...
    im2 = q < min(q(:))+ 0.3;%.45;%  make binary image to use regionprops
    

%     q = fast_order_parameter(Ang,5,5);
%     im2 = q < min(q(:))+ 0.8;
    s = regionprops('table', im2,'centroid');
    scaling = size(Ang,1)/size(q,1);% !!! delete or add scaling according to order parameter algorithm !!!
%     figure(98);
%     imshow(q);hold on
%     scatter(s.Centroid(:,1),s.Centroid(:,2));
  %%  
    % --------------------------------------------------------------------
    ...
    ps_x =  s.Centroid(:,1);
    ps_y =  s.Centroid(:,2);    
%     ps_x = scaling * s.Centroid(:,1);
%     ps_y = scaling * s.Centroid(:,2);
...
%             plocPsi_vec(ii,1)  = nanmean(nanmean(180/pi*plocPsi));
         TF = ~isnan(plocPsi);
         plocPsi_vec(i,1)  = 180/pi*circ_mean(plocPsi(TF));
    end
...
% R_theta = .5 * atan2(sub_ny1, sub_nx1);
% sub_nx = cos(R_theta);
% sub_ny = sin(R_theta);
...
%% PLOT VELOCITY COMPOUNDS ALONG THE DEFECT AXIS
figure(39)
midF =  .5*round(size(sub_PIV_u,1));
plot(mean(sub_PIV_u(midF-5:midF+5,:),1)); hold on
plot(mean(sub_PIV_v(midF-5:midF+5,:),1)); hold off
figure(39)
%% PLOT Ek over time
px_size = .74;
frame_per_hr = 4;
...
for jj=1:size(Sorted_Orient_width,1)
...
%     ii = 49;
    ...
    w_lc(jj,1) = Sorted_Orient_width(jj,2); % width
    w_lc(jj,2) = 2*(Ev\Ek)^.5;  % width
end
figure(25); scatter(w_lc(:,1),w_lc(:,2),'filled');
%% Check for specific width
widthChoice = 465; dw = .05*widthChoice;
jjS = find(Sorted_Orient_width(:,2)<widthChoice+dw &...
    Sorted_Orient_width(:,2)>widthChoice-dw, 10,'first')
jj = jjS(end)
...
figure(23);
c1=jet(size(Ek,1));
scatter(Ev,Ek,5,c1,'filled');hold off
...
time = (1:size(u))'/4;
figure(22);
yyaxis left; plot(time,Ek);hold on
yyaxis right; plot(time,Ev);hold on
...
ff = 5;
filt = fspecial('gaussian',ff,ff);
    uuF = imfilter(uu, filt);
    vvF = imfilter(vv, filt);
    vstep = 2; %hold on
q0 = quiver(Xorig(1:vstep:end,1:vstep:end),Yorig(1:vstep:end,1:vstep:end),...
    uuF(1:vstep:end,1:vstep:end),vvF(1:vstep:end,1:vstep:end),1.5);

% q0 = quiver(Xu(1:step:end,1:step:end),Yu(1:step:end,1:step:end),...
%     u_interp(1:step:end,1:step:end),v_interp(1:step:end,1:step:end),3);
q0.LineWidth = .5;
